...
const { extraerResultadosLoteria } = require("../helpers/extraerResultadosLoteria")
...
const elegirGanadores = async () => {
    ...
    const {data: apiSorteo} = await axios.get("https://api-resultadosloterias.com/api/results/2024-06-08")
    const loteriasParaExtraer = ["Boyaca", "Pijao", "Cauca"];
    const resultados = extraerResultadosLoteria(loteriasParaExtraer, apiSorteo.data)
    ...
    for(let i = 0; i < sorteos.length; i++){
        ...
        if(sorteos[i].fechaSorteo == ayer){
            ...
            const numeroGanadorUno = resultados[0].result.toString().slice(-cantidadDeNumeros).padStart(cantidadDeNumeros,"0")
            const numeroGanadorTres = resultados[1].result.toString().slice(-cantidadDeNumeros).padStart(cantidadDeNumeros,"0")
            const numeroGanadorDos = resultados[2].result.toString().slice(-cantidadDeNumeros).padStart(cantidadDeNumeros,"0")
            ...
            const ganadorUno = sorteo.tickets.find(ticket => String(ticket.numero).padStart(cantidadDeNumeros,"0") == numeroGanadorUno)
            const ganadorDos = sorteo.tickets.find(ticket => String(ticket.numero).padStart(cantidadDeNumeros,"0") == numeroGanadorDos)
            const ganadorTres = sorteo.tickets.find(ticket => String(ticket.numero).padStart(cantidadDeNumeros,"0") == numeroGanadorTres)
